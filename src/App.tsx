  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans relative overflow-hidden">
      ...
      <main className="pb-24 pt-8 px-4 flex flex-col items-center justify-center min-h-[80vh] relative">
        ...
        {/* SVG pour les lignes de connexion (Background) */}
        {/* Note: C'est une approximation visuelle pour relier les boites comme sur ton dessin */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" style={{ stroke: '#cbd5e1', strokeWidth: 2 }}>
          {/* Ligne Bac Gen -> Prepa */}
          <line x1="50%" y1="20%" x2="50%" y2="40%" />
          
          {/* Lignes College -> Bacs (Le trident) */}
          <path d="M50% 80% C 50% 70%, 15% 70%, 15% 55%" fill="none" /> {/* Vers Pro */}
          <path d="M50% 80% L 50% 55%" fill="none" />                   {/* Vers Gen */}
          <path d="M50% 80% C 50% 70%, 85% 70%, 85% 55%" fill="none" /> {/* Vers Tech */}
        </svg>

        {/* NIVEAU 1 : HAUT (Prépas) */}
        <div className="z-10 w-full max-w-xs flex justify-center mb-12">
          <NodeCard 
            title="Prépa" 
            subtitle="Choisir un établissement" 
            type="top"
          />
        </div>

        {/* NIVEAU 2 : MILIEU (Les Bacs) */}
        <div className="z-10 w-full grid grid-cols-3 gap-2 mb-20">
          <div className="flex justify-center mt-8"> {/* Décalé vers le bas pour style "Pro" */}
            <NodeCard title="Pro" subtitle="Etablissement" compact />
          </div>
          <div className="flex justify-center -mt-4"> {/* Remonté pour "Général" */}
             <NodeCard title="Bac Général" subtitle="Choisir un établissement" highlight />
          </div>
          <div className="flex justify-center mt-8">
             <NodeCard title="Bac Tech" subtitle="Choisir..." compact />
          </div>
        </div>

        {/* NIVEAU 3 : BAS (Collège) */}
        <div className="z-10 w-full max-w-xs flex justify-center">
          <NodeCard 
            title="Collège" 
            subtitle="Voir un établissement" 
            icon={<MapPin size={16} />}
          />
        </div>
      ...
      {/* --- FLOATING ACTION BUTTON (PARTAGER) --- */}
...
const NodeCard = ({ title, subtitle, highlight, compact, icon, type }: any) => {
  return (
    <div className={`
      ...
      ${highlight ? 'border-blue-600 ring-2 ring-blue-100 scale-105' : 'border-gray-300'}
    ...
    `}>
      ...
      <h3 className={`font-bold text-gray-800 ${compact ? 'text-xs' : 'text-sm'}`}>
        {title}
      </h3>
      
      {/* Le bouton "Pillule" interne */}
      <div className={`mt-2 bg-gray-100 text-gray-500 rounded-full px-2 py-1 truncate max-w-full ${compact ? 'text-[8px]' : 'text-[10px]'}`}>
        {subtitle}
      </div>

      {/* Petit point de connexion visuel (Decoratif) */}
